...
func Start(ctx context.Context, t *testing.T) (*gorm.DB, sqlmock.Sqlmock, error) {
	...
	postgresContainer, err := testcontainerutils.StartContainer(ctx, containerReq)
	if err != nil {
		return nil, nil, testcontainerutils.TerminateOnErr(ctx, postgresContainer, err, "failed to start PostgreSQL container")
	}

	t.Cleanup(func() {
		if err := testcontainerutils.TerminateContainer(ctx, postgresContainer); err != nil {
	...
	if err != nil {
		return nil, nil, testcontainerutils.TerminateOnErr(ctx, postgresContainer, err, "failed to create ORM connection")
	}

	mock, err := setupSQLMock()
	if err != nil {
		return nil, nil, testcontainerutils.TerminateOnErr(ctx, postgresContainer, err, "failed to create mock object")
	}

	if err := loadSeed(DB); err != nil {
		return nil, nil, testcontainerutils.TerminateOnErr(ctx, postgresContainer, err, "failed to load seed data")
	}
